                ...
                for k in range (0,10):
                    try:
                        # if (RPI_ON&~(interrupt_flag)):
                            ...
                            #Leitura de Corrente -  Calculo da media da corrente
                            for x in range(1,11):
                                bus.write_byte(address2, 1)
                                bus.read_byte(address2)
                                current_temp = bus.read_byte(address2)
                                current += current_temp*5.0/255.0
                            current = current/x


                            #Leitura de Tensao - calculo da media da tensao
                            for x in range(1,11):
                                bus.write_byte(address2, 0)
                                bus.read_byte(address2)
                                voltage_temp = bus.read_byte(address2)
                                voltage += voltage_temp*5.0/255.0
                            voltage = voltage/x
                            ...
                            for x in range(1,11):
                                ...
                                bus.read_byte(address2)
                                temperatura_aux = bus.read_byte(address2)
                                temperatura_aux = (0.6398*temperatura_aux)+9.686
                            ...
                            # realCurrent = 0.001664*dac_aux_bleh**6 - 0.04063*dac_aux_bleh**5 + 0.3828*dac_aux_bleh**4 - 1.798*dac_aux_bleh**3 + 4.435*dac_aux_bleh**2 - 5.199*dac_aux_bleh + 2.485
                            ...
                            wb1.save('calibracao_data_v0.xls')
                            cont = 0
                            break
