...
sheet1.write(0,0,"Experimento Prof Cesar")
sheet1.write(1,0,"Min")
sheet1.write(1,1,"Seg")
sheet1.write(1,2,"Potencia")
sheet1.write(1,3,"Impedancia")
sheet1.write(1,4,"Temperatura") 
sheet1.write(1,5,"Tensao")
sheet1.write(1,6,"Corrente")   
...
class Ui_manualDialog(object):
    ...
    def retranslateUi(self, thirdDialog):
        ...
        self.label_2.setText(_translate("thirdDialog", "SOFIA", None))
        ...
        self.label_9.setText(_translate("thirdDialog", "T       ºC", None))
        self.label_V.setText(_translate("thirdDialog", "Tempo", None))
...
def getPower(threadname, delay):
    ...
    global var0,sheet1,cont,first_2_seconds
    global time_before, time_beginning, minute, seconds, stop_press, initial_press,time_old, restart, time_off, time_now
    ...
    while (1):
                ...
                for k in range (0,10):
                    try:
                        if (RPI_ON):
                            ...
                            #voltage 5w feb/02 MELHOR
                            voltage = 0.0061*voltage_filtrada + 10.008

                            #FEB/08 10w
                            #voltage = 0.0061*voltage_filtrada + 10.417

                            #teste 30w
                            #voltage = 0.0056*voltage_filtrada + 10.294
                            ...
                            #teste 5w feb/02   MELHOR
                            current = 0.00004*current_filtrada + 0.0692
                            
                            #feb/08 10w
                            #current = 0.00004*current_filtrada + 0.0449

                            #teste 30w
                            #current = 0.00004*current_filtrada + 0.0094
                            ...
                            for x in range(0,15): #Leitura de Temperatura
                                temp_aux = adc.read_adc(2, gain=GAIN)
                                temperature += temp_aux
                            temperature = float(temperature)/(x+1)
                            ...
                            ui.lcdCurrent.display(temperature)
                            time.sleep(1)
            ...
            if seconds < 10:
                str_count = str(minute-1) + ':0' + str(int(seconds))

            else:
                str_count = str(minute-1) + ':' + str(int(seconds))
            ui.lcdVoltage.display(str_count) 


            if cont == 500:
                #Salvando valores na tabela excel
                sheet1.write(var0,0,minute-1)
                sheet1.write(var0,1,seconds)
                sheet1.write(var0,2,power)
                sheet1.write(var0,3,impedancia)
                sheet1.write(var0,4,temperature)
                sheet1.write(var0,5,voltage)
                sheet1.write(var0,6,current)
                #print "Index:" + str(var0)
                var0 += 1   
                wb1.save('Dados_Termometria.xls')
